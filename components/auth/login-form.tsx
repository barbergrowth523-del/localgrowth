...
export function LoginForm({ initialMode = 'login' }: { initialMode?: 'login' | 'signup' }) {
  ...
  return <main className="flex min-h-screen w-full bg-slate-950 text-white">
    <section className="hidden lg:flex w-1/2 flex-col justify-center px-12 xl:px-20 bg-slate-900 border-r border-slate-800 relative overflow-hidden"><div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-emerald-500/10 blur-[160px] rounded-full pointer-events-none" />
      <div className="relative z-10 flex items-center gap-3 font-semibold tracking-tight"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-emerald-400 text-slate-950"><Scissors size={19} /></span> <span className="text-2xl font-bold">BarberGrowth</span></div>
      <div className="relative z-10 w-full max-w-2xl pb-12"><p className="mt-16 mb-5 text-sm font-bold uppercase tracking-[.2em] text-emerald-400">Relacionamento que mantém a cadeira cheia.</p><h1 className="text-6xl font-semibold leading-[.98] tracking-[-.06em] text-white">Seu cliente não deveria precisar lembrar de você.</h1><p className="mt-7 text-lg leading-8 text-slate-400">Organize seus clientes, entenda quem está sumido e transforme uma mensagem em mais um horário marcado.</p><ul className="mt-7 space-y-3 text-sm text-slate-300"><li className="flex items-center gap-3"><Check size={16} className="shrink-0 text-emerald-400" />Integração instantânea com WhatsApp</li><li className="flex items-center gap-3"><Check size={16} className="shrink-0 text-emerald-400" />Mapeamento inteligente de evasão</li><li className="flex items-center gap-3"><Check size={16} className="shrink-0 text-emerald-400" />Retorno sobre investimento imediato</li></ul><div className="mt-12 w-full rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-md"><div className="flex gap-1 text-amber-400">{[1, 2, 3, 4, 5].map(star => <Star key={star} size={14} fill="currentColor" />)}</div><p className="mt-4 text-sm italic leading-6 text-slate-300">&quot;O sistema encontrou R$ 850 perdidos na minha base de clientes logo no primeiro dia de uso. O investimento se pagou na mesma hora.&quot;</p><p className="mt-4 text-sm font-semibold text-white">Marcos, Carvalhos Barber Shop</p></div></div>
    </section>
    <section className="flex w-full lg:w-1/2 flex-col justify-center items-center px-8 bg-slate-950"><div className="w-full max-w-[390px]">
