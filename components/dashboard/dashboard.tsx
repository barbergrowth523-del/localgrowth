...
export function Dashboard({ userEmail }: { userEmail: string }) {
  ...
  return <main className="min-h-screen bg-slate-100"><header className="border-b border-slate-200/80 bg-white shadow-sm"><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-10"><div className="flex items-center gap-3 font-semibold"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#1c6b49] text-white"><Scissors size={17} /></span> BarberGrowth</div><div className="flex items-center gap-4"><span className="hidden rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700 sm:block">{userEmail}</span><button onClick={signOut} className="rounded-lg p-2 text-[#78847c] transition hover:bg-[#eef2ed]" title="Sair"><LogOut size={17} /></button></div></div></header>
    <div className="mx-auto max-w-7xl px-6 py-10 lg:px-10"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-sm font-medium text-[#1c6b49]">Visão geral</p><h1 className="mt-2 text-4xl font-semibold tracking-[-.05em]">Bom dia, barbeiro.</h1><p className="mt-2 text-[#78847c]">Veja quem está esperando por um novo corte.</p></div><div><input ref={fileRef} type="file" accept=".csv,text/csv" onChange={importCsv} className="hidden" /><button onClick={() => fileRef.current?.click()} className="flex items-center justify-center gap-2 rounded-xl bg-[#1c6b49] px-5 py-3.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#15563a]"><FileUp size={17} /> Importar planilha CSV</button></div></div>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-3"><Stat icon={<Users size={18} />} label="Total de clientes" value={clients.length} /><RiskStat value={atRisk} /><Stat icon={<CalendarDays size={18} />} label="Cortes este mês" value={clients.filter(c => new Date(c.last_cut_at).getMonth() === new Date().getMonth()).length} /></div>
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: number }) {
  return <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm"><div className="mb-5 flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600">{icon}</div><p className="text-sm text-slate-500">{label}</p><p className="mt-1 text-3xl font-semibold tracking-[-.04em] text-slate-900">{value}</p></div>
}
function RiskStat({ value }: { value: number }) {
  return <div className="rounded-xl border border-rose-200/80 bg-rose-50/50 p-5 shadow-sm"><div className="mb-5 flex h-9 w-9 items-center justify-center rounded-lg bg-rose-100 text-rose-600"><AlertCircle size={18} /></div><p className="text-sm font-medium text-rose-700">Faturamento em risco</p><p className="mt-1 text-3xl font-bold tracking-[-.04em] text-rose-600">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(value)}</p><p className="mt-1 text-xs text-rose-700/70">Clientes sumidos x R$ 60,00</p></div>
}
