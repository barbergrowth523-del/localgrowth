...
export default function ConfiguracoesPage() {
  ...
  async function saveCapacity() {
    const value = Number(cadeirasSimultaneas)
    if (!Number.isInteger(value) || value < 1 || value > 50) { setStatus('Informe uma capacidade entre 1 e 50 cadeiras.'); return }
    const supabase = createClient()
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) { setStatus('Sua sessao expirou. Entre novamente.'); return }
    const { error } = await supabase.from('perfis_barbearia').upsert({ id: user.id, cadeiras_simultaneas: value })
    setStatus(error ? 'Erro ao salvar capacidade: ' + error.message : 'Capacidade salva com sucesso!')
  }
  ...
  async function saveSchedule() {
    ...
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) { setStatus('Sua sessao expirou. Entre novamente.'); return }
    const { error } = await supabase.from('expedientes').upsert(schedule.map((item) => ({ ...item, user_id: user.id })), { onConflict: 'user_id,dia_semana' })
    setStatus(error ? 'Erro ao salvar horarios: ' + error.message : 'Horarios de atendimento salvos com sucesso!')
  }
