...
export default function ConfiguracoesPage() {
  ...
  function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    alert('Configuracoes avancadas salvas com sucesso!')
  }

  return (
    <div className="mx-auto w-full max-w-5xl p-8 lg:p-12">
      <div className="mb-8"><h1 className="flex items-center gap-3 text-3xl font-bold text-white"><Settings className="h-8 w-8 text-emerald-400" /> Configuracoes do Sistema</h1><p className="mt-1 text-sm text-slate-400">Personalize as automacoes, regras de resgate e informacoes do seu estabelecimento.</p></div>
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="flex justify-end pt-2"><button type="submit" className="flex items-center gap-2 rounded-xl bg-emerald-500 px-8 py-4 font-bold text-slate-950 shadow-lg shadow-emerald-500/10 transition hover:bg-emerald-600"><Save className="h-4 w-4" /> Salvar Todas as Configuracoes</button></div>
      </form>
    </div>
  )
}
