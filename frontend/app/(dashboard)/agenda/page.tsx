...
type Service = 'Corte' | 'Barba' | 'Corte e Barba'
type Client = { id: string; nome: string; telefone: string }
type Appointment = { id: string; cliente_id: string; data_agendamento: string; hora_agendamento: string; servico: Service; status: Status }
type FormState = { clienteId: string; date: string; time: string; service: Service }

const services: Service[] = ['Corte', 'Barba', 'Corte e Barba']
const today = new Date().toISOString().slice(0, 10)
const emptyForm: FormState = { clienteId: '', date: today, time: '09:00', service: 'Corte' }
...
export default function AgendaPage() {
  ...
  async function loadAgendaData() {
    ...
    const [clientsResult, appointmentsResult] = await Promise.all([
      ...
      supabase.from('agendamentos').select('id,cliente_id,data_agendamento,hora_agendamento,servico,status').eq('user_id', user.id).order('data_agendamento').order('hora_agendamento'),
    ])
    if (clientsResult.error || appointmentsResult.error) { setStatus('Erro ao carregar a agenda.'); return }
  ...
  function openModal() {
    setForm({ ...emptyForm, date: selectedDate, clienteId: clients[0]?.id ?? '' })
    setModalOpen(true)
    setStatus('')
  }
  ...
  async function createAppointment(event: FormEvent<HTMLFormElement>) {
    ...
    if (!form.clienteId) { setStatus('Selecione um cliente para continuar.'); return }
    setSaving(true); setStatus('')
    const supabase = createClient()
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) { setStatus('Sua sessao expirou. Entre novamente.'); setSaving(false); return }
    const { error } = await supabase.from('agendamentos').insert({ user_id: user.id, barbearia_id: user.id, cliente_id: form.clienteId, data_agendamento: form.date, hora_agendamento: form.time, servico: form.service, status: 'Confirmado' })
