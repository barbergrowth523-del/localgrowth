...
export default function AssinaturaPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      else { setMessage('Teste gratis ativado por 7 dias. Nenhum valor sera cobrado hoje.'); window.setTimeout(() => { window.location.href = isOnboardingOffer ? '/dashboard?tour=1&tourStep=0' : '/dashboard' }, 900) }
  ...
  return (
    <div data-page="subscription" className="mx-auto w-full max-w-7xl px-4 py-6 sm:p-6 md:p-8 lg:p-12">
      <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div><h1 className="flex items-center gap-3 text-3xl font-bold text-white"><Sparkles className="h-8 w-8 text-emerald-400" /> Planos {BRAND_NAME}</h1><p className="mt-2 text-sm text-slate-400">{isOnboardingOffer ? 'Comece agora com 7 dias gratis. Nenhum valor sera cobrado hoje.' : 'Escolha o plano certo para recuperar mais clientes e aumentar seu faturamento.'}</p></div>
      ...
      {isOnboardingOffer && (
        <section className="mb-6 overflow-hidden rounded-2xl border border-emerald-400/30 bg-gradient-to-br from-emerald-500/15 via-slate-900 to-slate-900 p-5 shadow-xl shadow-emerald-500/10 sm:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-3xl">
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-300">Bem-vindo ao Prontusfy</p>
              <h2 className="mt-2 text-2xl font-bold tracking-tight text-white sm:text-3xl">Teste tudo por 7 dias, sem cobrar nada hoje.</h2>
              <p className="mt-2 text-sm leading-6 text-slate-300">Escolha seu plano, cadastre o cartao com seguranca e use a plataforma imediatamente. A primeira cobranca so acontece apos o periodo de teste. Voce pode cancelar a renovacao quando quiser, sem multa.</p>
            </div>
            <button type="button" onClick={() => { setMetodo('CREDIT_CARD'); document.getElementById('checkout-cartao')?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-emerald-400 px-5 py-3.5 text-sm font-bold text-slate-950 shadow-lg shadow-emerald-500/20 transition hover:bg-emerald-300">
              Escolher plano <ArrowRight className="h-4 w-4" />
            </button>
          </div>
          <div className="mt-5 grid gap-2 text-xs font-semibold text-emerald-100 sm:grid-cols-3">
            <span className="flex items-center gap-2 rounded-lg border border-emerald-400/15 bg-slate-950/30 px-3 py-2"><ShieldCheck className="h-4 w-4 text-emerald-400" /> Zero risco</span>
            <span className="flex items-center gap-2 rounded-lg border border-emerald-400/15 bg-slate-950/30 px-3 py-2"><CheckCircle2 className="h-4 w-4 text-emerald-400" /> Nenhum valor cobrado hoje</span>
            <span className="flex items-center gap-2 rounded-lg border border-emerald-400/15 bg-slate-950/30 px-3 py-2"><RefreshCw className="h-4 w-4 text-emerald-400" /> Cancele quando quiser</span>
          </div>
        </section>
      )}
      ...
      {isOnboardingOffer && (
        <section className="mb-7 rounded-2xl border border-emerald-400/25 bg-slate-900 p-4 shadow-lg shadow-emerald-500/5 sm:p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div><p className="text-sm font-bold text-white">Plano {plan.name.replace('Plano ', '')} selecionado</p><p className="mt-1 text-xs text-slate-400">Seu acesso e liberado logo apos a validacao do cartao. Primeira cobranca somente ao fim dos 7 dias gratis.</p></div>
            <button type="button" onClick={() => { setMetodo('CREDIT_CARD'); document.getElementById('checkout-cartao')?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-emerald-400 px-5 py-3.5 text-center text-sm font-bold text-slate-950 transition hover:bg-emerald-300">Quero meus 7 Dias Gratis e Acessar a Plataforma <ArrowRight className="h-4 w-4" /></button>
          </div>
        </section>
      )}
