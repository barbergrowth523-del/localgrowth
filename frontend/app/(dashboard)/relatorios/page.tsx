...
type Appointment = { barbeiro_id: string | null; servico_id: string | null; status: string }
...
export default function RelatoriosPage() {
  ...
  useEffect(() => {
    async function loadReports() {
      ...
      const [barbersResult, appointmentsResult, servicesResult] = await Promise.all([
        supabase.from('barbeiros').select('id,nome,comissao_percentual').eq('user_id', user.id).order('nome'),
        supabase.from('agendamentos').select('barbeiro_id,servico_id,status').eq('user_id', user.id),
  ...
  const filtered = useMemo(() => appointments.filter((item) => selectedBarber === 'all' || item.barbeiro_id === selectedBarber), [appointments, selectedBarber])
