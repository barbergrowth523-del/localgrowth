...
import { Award, Eye, EyeOff, Lock, Mail, MapPin, Phone, Save, Shield, User } from 'lucide-react'
import { FormEvent, useEffect, useState } from 'react'
import { BRAND_NAME } from '@/lib/brand'
import { createClient } from '@/lib/supabase/client'

type Profile = { name: string; email: string; phone: string; cidade: string; endereco: string; currentPassword: string; newPassword: string }

export default function PerfilPage() {
  const [profile, setProfile] = useState<Profile>({ name: 'Samuel Santos', email: 'barbergrowth523@gmail.com', phone: '(74) 98888-7777', cidade: 'Jacobina, BA', endereco: 'Rua Principal, 100', currentPassword: '', newPassword: '' })
  const [status, setStatus] = useState('')
  const update = (key: keyof Profile, value: string) => setProfile((current) => ({ ...current, [key]: value }))

  useEffect(() => { void (async () => { const { data } = await createClient().auth.getUser(); if (data.user?.email) setProfile((current) => ({ ...current, email: data.user.email ?? current.email })) })() }, [])

  async function handleUpdate(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setStatus(''); const supabase = createClient()
    if (profile.currentPassword || profile.newPassword) {
      if (!profile.currentPassword || !profile.newPassword) return setStatus('Informe a senha atual e a nova senha.')
      if (profile.newPassword.length < 6) return setStatus('A nova senha deve ter no minimo 6 caracteres.')
      const { data: userData } = await supabase.auth.getUser()
      if (!userData.user?.email) return setStatus('Sessao invalida. Entre novamente.')
      const { error: authError } = await supabase.auth.signInWithPassword({ email: userData.user.email, password: profile.currentPassword })
      if (authError) return setStatus('A senha atual nao confere.')
    ...
    setStatus('Informacoes de perfil atualizadas com sucesso!')
  }

  return <div data-page="profile" className="mx-auto w-full max-w-5xl px-4 py-6 sm:p-6 md:p-8 lg:p-12">
    ...
    <form onSubmit={handleUpdate} className="space-y-6">{status && <p className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300">{status}</p>}
...
function IconField({ icon, label, value, onChange, type = 'text' }: { icon: React.ReactNode; label: string; value: string; onChange: (value: string) => void; type?: string }) { return <label className="block text-xs font-medium text-slate-400">{label}<div className="relative mt-2"><span className="absolute left-3.5 top-3.5 text-slate-500">{icon}</span><input type={type} value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-xl border border-slate-800 bg-slate-950 py-3.5 pl-10 pr-4 text-sm text-white outline-none transition focus:border-emerald-500" /></div></label> }
function Field({ label, value, onChange, placeholder, type, required }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; type: string; required: boolean }) { const [visible, setVisible] = useState(false); const isPassword = type === 'password'; return <label className="block text-xs font-medium text-slate-400">{label}<div className="relative mt-2"><input required={required} type={isPassword && !visible ? 'password' : 'text'} placeholder={placeholder} value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-xl border border-slate-800 bg-slate-950 p-3.5 pr-12 text-sm text-white outline-none transition focus:border-emerald-500" />{isPassword && <button type="button" onClick={() => setVisible((current) => !current)} aria-label={visible ? 'Esconder senha' : 'Mostrar senha'} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-emerald-400">{visible ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>}</div></label> }
