...
export async function POST(request: Request) {
  try {
    ...
    if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
      return NextResponse.json({ error: 'Avaliacao nao configurada.' }, { status: 500 })
    }
    ...
    const { error: reviewError } = await supabase
      .from('avaliacoes')
      .upsert({ agendamento_id: appointmentId, estrelas: stars, comentario: comment }, { onConflict: 'agendamento_id' })
    if (reviewError) return NextResponse.json({ error: 'Nao foi possivel salvar a avaliacao.' }, { status: 500 })

    const { error: appointmentError } = await supabase
      .from('agendamentos')
      .update({ nota_avaliacao: stars, comentario_avaliacao: comment })
      .eq('id', appointmentId)
    if (appointmentError) return NextResponse.json({ error: 'Avaliacao salva, mas nao foi possivel atualizar o agendamento.' }, { status: 500 })
