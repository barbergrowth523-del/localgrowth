...
export async function GET() {
  ...
  let { data: member } = await supabase.from('equipe').select('id,user_id,auth_user_id,nome,comissao_percentual,email').eq('auth_user_id', user.id).maybeSingle()
  if (!member && user.email) {
    const fallback = await supabase.from('equipe').select('id,user_id,auth_user_id,nome,comissao_percentual,email').ilike('email', user.email).maybeSingle()
