...
export function SupportChat({ barbershopName }: { barbershopName?: string | null }) {
  ...
  return <>
    {open && <section role="dialog" aria-label="Concierge Prontusfy" className="fixed bottom-36 right-3 z-50 flex h-[min(72vh,600px)] w-[calc(100vw-1.5rem)] max-w-md flex-col overflow-hidden rounded-3xl border border-emerald-500/20 bg-slate-950 shadow-2xl shadow-emerald-950/40 lg:bottom-24 lg:right-6">
    ...
    <button type="button" onClick={() => setOpen((current) => !current)} aria-label={open ? 'Fechar concierge' : 'Abrir Concierge Prontusfy'} className="fixed bottom-20 right-4 z-50 flex h-14 w-14 items-center justify-center rounded-full border border-emerald-200/20 bg-gradient-to-br from-emerald-300 via-emerald-400 to-teal-600 text-slate-950 shadow-xl shadow-emerald-500/30 transition hover:scale-105 hover:shadow-emerald-400/40 lg:bottom-6 lg:right-6">{open ? <X className="h-6 w-6" /> : <Crown className="h-6 w-6" />}</button>
  </>
}
