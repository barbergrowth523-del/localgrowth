...
export function SupportChat({ barbershopName }: { barbershopName?: string | null }) {
  ...
  return <>
    {open && <section role="dialog" aria-label="Concierge Prontusfy" className="fixed bottom-36 right-2 z-50 flex h-[min(72vh,600px)] w-[calc(100vw-1rem)] max-w-[380px] sm:right-3 sm:w-[380px] sm:max-w-sm flex-col overflow-hidden rounded-3xl border border-emerald-500/20 bg-slate-950 shadow-2xl shadow-emerald-950/40 lg:bottom-24 lg:right-6">
    ...
    <div className="fixed bottom-20 right-4 z-50 flex items-center gap-3 lg:bottom-6 lg:right-6">{!open && inviteVisible && <div className="flex max-w-[240px] items-start gap-1 rounded-2xl border border-emerald-500/20 bg-slate-900/95 px-3 py-2 text-right text-xs font-medium leading-5 text-emerald-50 shadow-xl shadow-black/30 backdrop-blur-xl"><span className="flex-1">Precisa de ajuda para faturar mais? Fale comigo <span aria-hidden="true">&#9986;</span></span><button type="button" onClick={() => setInviteVisible(false)} aria-label="Fechar convite" className="-mr-1 -mt-1 rounded-md p-1 text-slate-500 transition hover:bg-slate-800 hover:text-white"><X className="h-3.5 w-3.5" /></button></div>}<button type="button" onClick={() => setOpen((current) => !current)} aria-label={open ? 'Fechar concierge' : 'Abrir Concierge Prontusfy'} className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-emerald-200/20 bg-gradient-to-br from-emerald-300 via-emerald-400 to-teal-600 text-slate-950 shadow-xl shadow-emerald-500/30 transition hover:scale-105 hover:shadow-emerald-400/40">{open ? <X className="h-6 w-6" /> : <Crown className="h-6 w-6" />}</button></div>
  </>
}
