...
export function PanelTopbar() {
  ...
  return <header className="sticky top-0 z-30 border-b border-slate-800 bg-slate-950/95 backdrop-blur-xl">
    <div className="relative flex h-16 w-full items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
      ...
      <Link href="/assinatura" className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-1 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2 py-1 text-[10px] font-semibold text-emerald-300 transition hover:border-emerald-400/60 hover:bg-emerald-500/20 sm:inline-flex"><Sparkles size={11} className="text-emerald-400" /><span>{planLabel} {subscriptionActive ? 'Ativo' : ''}</span></Link>
