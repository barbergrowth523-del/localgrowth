  ...
  return <header className="sticky top-0 z-30 border-b border-slate-800 bg-slate-950/95 backdrop-blur-xl">
    <div className="flex h-16 w-full items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex min-w-0 items-center justify-end gap-2 sm:gap-3">
        ...
        <Link href="/assinatura" className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1.5 text-[11px] font-semibold text-emerald-300 transition hover:border-emerald-400/60 hover:bg-emerald-500/20"><Sparkles size={13} className="text-emerald-400" /><span className="hidden md:inline">{planLabel} {subscriptionActive ? 'Ativo' : ''}</span><span className="md:hidden">{formatPlan(plan)}</span></Link>
