...
import { Activity, AlertCircle, CalendarDays, CheckCircle2, ChevronRight, Clipboard, FileUp, Cake, LoaderCircle, MessageCircle, Search, Sparkles, TrendingUp, Users, Wallet, X } from 'lucide-react'
...
export function Dashboard() {
  ...
  const filteredClientes = useMemo(() => { const source = tourDemo ? [demoClient, ...clients] : clients; const normalizedSearch = searchTerm.trim().toLowerCase(); return source.filter(client => { const matchesSearch = (client.name + ' ' + client.phone).toLowerCase().includes(normalizedSearch); const normalizedFrequency = client.frequency?.toLowerCase(); const matchesFrequency = frequencyFilter === 'todos' || !normalizedFrequency || normalizedFrequency === frequencyFilter; return matchesSearch && matchesFrequency }) }, [clients, searchTerm, frequencyFilter, tourDemo])
  ...
  const profileName = user?.email?.split('@')[0] || 'barbeiro'
  return <main data-dashboard className="min-h-screen bg-slate-950">
    <div className="mx-auto max-w-[1440px] px-4 py-6 sm:px-6 lg:px-10"><div className="dashboard-intro mb-7 flex flex-col items-start justify-between gap-5 lg:flex-row lg:items-center"><div><p className="text-sm font-medium text-emerald-400">Visao geral</p><h1 className="mt-1 text-2xl font-bold tracking-tight text-white sm:text-3xl">Bom dia, {profileName}</h1><p className="mt-1 text-sm text-slate-400 sm:text-base">Veja quem esta esperando por um novo corte.</p></div><div><input ref={fileRef} type="file" accept=".csv,text/csv" onChange={importCsv} className="hidden" /><button disabled={isUploading} onClick={() => fileRef.current?.click()} className="flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-3.5 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-60"><FileUp size={17} /> {isUploading ? 'Importando...' : 'Importar planilha CSV'}</button></div></div>
...
function MoneyOnTableCard({ value, sumidoCount }: { value: number; sumidoCount: number }) {
  ...
  return <div data-light-tone="green" className="relative overflow-hidden rounded-xl border border-slate-800 bg-slate-900 p-4 text-white shadow-sm"><div className="absolute -right-10 -top-16 h-40 w-40 rounded-full bg-emerald-400/15 blur-3xl" /><div className="relative flex h-full flex-col justify-between gap-4"><span data-light-only aria-hidden="true" className="h-9 w-9 items-center justify-center rounded-xl"><Wallet size={17} /></span><p className="text-sm font-medium text-slate-300">Dinheiro na Mesa</p><div><p className="text-2xl font-bold tracking-[-.04em] text-emerald-300">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(displayedValue)}</p><p className="mt-1 text-xs text-slate-300">{countLabel(sumidoCount, 'cliente sumido', 'clientes sumidos')}</p></div></div></div>
}
function Stat({ icon, label, value, tone }: { icon: React.ReactNode; label: string; value: number; tone: 'purple' | 'blue' }) {
  const displayedValue = useCountUp(value)
  return <div data-light-tone={tone} className="h-full rounded-xl border border-slate-800 bg-slate-900 p-4 shadow-sm backdrop-blur-md"><div data-light-icon className="mb-3 flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-400">{icon}</div><p className="text-sm font-medium text-slate-500">{label}</p><p className="mt-1 text-2xl font-bold tracking-[-.04em] text-white">{displayedValue}</p></div>
}
function RoiBadge({ multiple, recoveries }: { multiple: number; recoveries: number }) {
  const displayedMultiple = useCountUp(multiple)
  return <div data-light-tone="roi" className="h-full rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4 shadow-sm"><span data-light-only aria-hidden="true" className="h-9 w-9 items-center justify-center rounded-xl"><TrendingUp size={17} /></span><p className="mt-3 text-sm font-medium text-slate-500">Retorno do Sistema</p><p className="mt-2 text-2xl font-bold text-emerald-300">{multiple === 0 ? 'Pronto para o primeiro resgate!' : 'O sistema ja se pagou ' + displayedMultiple + ' vezes!'}</p><p className="mt-1 text-xs text-emerald-300/80">Estimativa com {countLabel(recoveries, 'resgate', 'resgates')} hipoteticos.</p></div>
}
...
function HealthBaseChart({ clients, alertDays, missingDays }: { clients: Client[]; alertDays: number; missingDays: number }) {
  ...
  return <div data-light-tone="health" className="mb-6 rounded-lg border border-slate-800 bg-slate-900 p-4 shadow-sm"><div className="mb-3 flex items-center gap-2"><span data-light-only aria-hidden="true" className="h-8 w-8 items-center justify-center rounded-lg"><Activity size={16} /></span><h3 className="text-sm font-semibold text-slate-300">Saude da Base de Clientes</h3></div><div className="mb-2 flex h-3 w-full overflow-hidden rounded-full"><div className="bg-emerald-500" style={{ width: retained + '%' }} /><div className="bg-amber-400" style={{ width: alert + '%' }} /><div className="bg-rose-500" style={{ width: risk + '%' }} /></div><div className="flex justify-between text-xs font-medium text-slate-500"><span className="inline-flex items-center gap-1.5"><span className="inline-block h-2 w-2 rounded-full bg-emerald-500" />{retained}% Retidos</span><span className="inline-flex items-center gap-1.5"><span className="inline-block h-2 w-2 rounded-full bg-amber-400" />{alert}% Em Alerta</span><span className="inline-flex items-center gap-1.5"><span className="inline-block h-2 w-2 rounded-full bg-rose-500" />{risk}% Em Risco</span></div></div>
}
