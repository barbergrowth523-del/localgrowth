...
import { AlertCircle, CalendarDays, CheckCircle2, ChevronRight, Clipboard, FileUp, Cake, LoaderCircle, LogOut, MessageCircle, Search, Scissors, Sparkles, Users, X } from 'lucide-react'
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: number }) {
  const displayedValue = useCountUp(value)
  return <div className="h-full rounded-xl border border-slate-200/80 bg-white/80 p-4 shadow-sm backdrop-blur-md"><div className="mb-3 flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600">{icon}</div><p className="text-sm font-medium text-slate-500">{label}</p><p className="mt-1 text-2xl font-bold tracking-[-.04em] text-slate-900">{displayedValue}</p></div>
}
function RoiBadge({ multiple, recoveries }: { multiple: number; recoveries: number }) {
  const displayedMultiple = useCountUp(multiple)
  return <div className="h-full rounded-xl border border-emerald-200/80 bg-emerald-50/70 p-4 shadow-sm"><p className="text-sm font-medium text-slate-500">Retorno do Sistema</p><p className="mt-2 text-2xl font-bold text-emerald-700">{multiple === 0 ? 'Pronto para o primeiro resgate!' : 'O sistema ja se pagou ' + displayedMultiple + ' vezes!'}</p><p className="mt-1 text-xs text-emerald-700/80">Estimativa com {countLabel(recoveries, 'resgate', 'resgates')} hipoteticos.</p></div>
}
...
function BirthdayBanner() {
  ...
  const template = 'Oi! Feliz anivers?rio, [nome]! Para comemorar, seu pr?ximo corte tem 15% de desconto. Quer reservar seu hor?rio?'
