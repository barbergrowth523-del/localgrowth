...
import { AlertCircle, CalendarDays, CheckCircle2, ChevronRight, Clipboard, FileUp, Cake, LoaderCircle, LogOut, MessageCircle, Search, Sparkles, User, Users, X } from 'lucide-react'
...
function ClientRow({ client, onSelect }: { client: Client; onSelect: () => void }) {
  ...
  return <tr onClick={onSelect} className={'cursor-pointer border-b border-slate-800 transition-colors hover:bg-slate-800/60 ' + (overdue ? 'bg-rose-500/5' : '')}>
    <td className="px-5 py-5 font-medium text-slate-200"><div className="flex items-center gap-3"><span className={'flex h-9 w-9 items-center justify-center rounded-full text-xs font-semibold ' + (overdue ? 'bg-rose-500/10 text-rose-300' : 'bg-emerald-500/10 text-emerald-300')}>{client.name.split(' ').map(x => x[0]).slice(0, 2).join('')}</span>{client.name}{vipAtRisk && <span className="ml-2 rounded-full bg-violet-500/10 px-2 py-1 text-[10px] font-semibold text-violet-300">VIP em risco</span>}</div></td>
    <td className="px-5 py-5 text-slate-400">{formatPhone(client.phone)}</td>
    <td className="px-5 py-5 text-slate-400">{formatDate(client.last_cut_at)}</td>
