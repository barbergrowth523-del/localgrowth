...
function GoogleIcon() {
  return <svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5"><path fill="#4285F4" d="M21.35 12.23c0-.7-.06-1.37-.18-2.02H12v3.82h5.24a4.48 4.48 0 0 1-1.94 2.94v2.45h3.14c1.84-1.7 2.91-4.2 2.91-7.19Z"/><path fill="#34A853" d="M12 21.75c2.63 0 4.84-.87 6.45-2.33l-3.14-2.45c-.87.58-1.98.93-3.31.93-2.54 0-4.7-1.72-5.47-4.03H3.29v2.53A9.75 9.75 0 0 0 12 21.75Z"/><path fill="#FBBC05" d="M6.53 13.87a5.86 5.86 0 0 1 0-3.74V7.6H3.29a9.75 9.75 0 0 0 0 8.8l3.24-2.53Z"/><path fill="#EA4335" d="M12 6.1c1.43 0 2.71.49 3.72 1.46l2.79-2.79C16.84 3.2 14.63 2.25 12 2.25a9.75 9.75 0 0 0-8.71 5.36l3.24 2.53C7.3 7.83 9.46 6.1 12 6.1Z"/></svg>
}

type PasswordFieldProps = { label: string; value: string; onChange: (value: string) => void; placeholder: string; required?: boolean; minLength?: number; showStrength?: boolean }
function PasswordField({ label, value, onChange, placeholder, required = true, minLength = 6, showStrength = false }: PasswordFieldProps) {
  const [visible, setVisible] = useState(false)
  const strength = value.length >= 10 ? 'strong' : value.length >= minLength ? 'medium' : 'weak'
  return <label className="block text-sm font-medium text-slate-200">{label}<div className="relative mt-2"><input required={required} minLength={minLength} type={visible ? 'text' : 'password'} value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-xl border border-slate-800 bg-slate-900 px-4 py-3.5 pr-12 text-base text-white outline-none ring-emerald-400 transition placeholder:text-slate-600 focus:ring-2" placeholder={placeholder} /><button type="button" onClick={() => setVisible((current) => !current)} aria-label={visible ? 'Esconder senha' : 'Mostrar senha'} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg p-1.5 text-slate-500 transition hover:bg-slate-800 hover:text-emerald-300">{visible ? <EyeOff size={18} /> : <Eye size={18} />}</button></div>{showStrength && value && <div className="mt-2"><div className="flex gap-1">{[1, 2, 3].map((part) => <span key={part} className={`h-1 flex-1 rounded-full ${strength === 'strong' || (strength === 'medium' && part < 3) || (strength === 'weak' && part === 1) ? strength === 'strong' ? 'bg-emerald-400' : 'bg-amber-400' : 'bg-slate-700'}`} />)}</div><p className="mt-1 text-xs text-slate-500">{strength === 'strong' ? 'Senha forte' : `Use no minimo ${minLength} caracteres`}</p></div>}</label>
}
...
export function LoginForm({ initialMode = 'login' }: { initialMode?: 'login' | 'signup' }) {
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.search); if (params.get('error')?.startsWith('oauth_')) setMessage('Nao foi possivel concluir o acesso com Google. Verifique se o provedor esta habilitado no Supabase.'); if (params.get('recovery') === '1' || window.location.hash.includes('type=recovery')) { setRecoveryOpen(true); setRecoveryStep('token') } }, [])
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (mode === 'signup') {
        ...
        window.sessionStorage.setItem('prontusfy-signup-email', normalizedEmail)
        window.sessionStorage.setItem('prontusfy-signup-otp-sent-at', String(Date.now()))
        window.location.assign('/verificar-email')
        return
      }
      ...
      if (user) void supabase.from('account_activity_events').insert({ user_id: user.id, event_type: 'login' })
      ...
      setMessage('Nao foi possivel concluir a solicitacao. Tente novamente.')
    } finally {
      setLoading(false)
    }
  }
  ...
  async function continueWithGoogle() {
    ...
    if (error) { setLoading(false); const text = error.message.toLowerCase(); setMessage(text.includes('provider') || text.includes('google') ? 'O acesso com Google ainda nao esta habilitado. Ative o provedor Google no Supabase Auth.' : 'Nao foi possivel iniciar o acesso com Google. Tente novamente.') }
  }

  async function sendRecoveryEmail(event: FormEvent) { event.preventDefault(); setRecoveryLoading(true); setRecoveryMessage(''); const { error } = await createClient().auth.resetPasswordForEmail(recoveryEmail, { redirectTo: `${window.location.origin}/esqueci-senha/verificar` }); setRecoveryLoading(false); if (error) return setRecoveryMessage(error.message); setRecoveryStep('token'); setRecoveryMessage('Enviamos um codigo de recuperacao para seu e-mail. Digite o codigo recebido e defina sua nova senha.') }
  async function updateRecoveredPassword(event: FormEvent) { event.preventDefault(); setRecoveryLoading(true); setRecoveryMessage(''); const supabase = createClient(); if (recoveryToken.trim()) { const { error } = await supabase.auth.verifyOtp({ email: recoveryEmail, token: recoveryToken.trim(), type: 'recovery' }); if (error) { setRecoveryLoading(false); return setRecoveryMessage('Codigo invalido ou expirado. Solicite um novo e-mail.') } } else { const { data } = await supabase.auth.getSession(); if (!data.session) { setRecoveryLoading(false); return setRecoveryMessage('Abra o link do e-mail ou cole o codigo recebido.') } } const { error } = await supabase.auth.updateUser({ password: recoveryPassword }); setRecoveryLoading(false); if (error) return setRecoveryMessage(error.message); setRecoveryMessage('Senha atualizada com sucesso. Voce ja pode entrar.'); setRecoveryStep('email'); setRecoveryToken(''); setRecoveryPassword('') }
  ...
  return <>
    <AuthLayout aside={<div className="mt-8"><p className="mb-4 text-sm font-bold uppercase tracking-[.2em] text-emerald-400">Relacionamento que mantem a cadeira cheia.</p><h1 className="text-5xl font-semibold leading-[.98] tracking-[-.06em] text-white xl:text-6xl">Seu cliente nao deveria precisar lembrar de voce.</h1><p className="mt-5 text-base leading-7 text-slate-400 xl:text-lg xl:leading-8">Organize seus clientes, entenda quem esta sumido e transforme uma mensagem em mais um horario marcado.</p><ul className="mt-6 space-y-3 text-sm text-slate-300"><li className="flex items-center gap-3"><Check size={16} className="shrink-0 text-emerald-400" />Integracao instantanea com WhatsApp</li><li className="flex items-center gap-3"><Check size={16} className="shrink-0 text-emerald-400" />Mapeamento inteligente de evasao</li><li className="flex items-center gap-3"><Check size={16} className="shrink-0 text-emerald-400" />Retorno sobre investimento imediato</li></ul><div className="mt-8 w-full rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-md sm:p-6"><div className="flex gap-1 text-amber-400">{[1, 2, 3, 4, 5].map((star) => <Star key={star} size={14} fill="currentColor" />)}</div><p className="mt-4 text-sm italic leading-6 text-slate-300">&quot;O sistema encontrou R$ 850 perdidos na minha base de clientes logo no primeiro dia de uso. O investimento se pagou na mesma hora.&quot;</p><p className="mt-4 text-sm font-semibold text-white">Marcos, Carvalhos Barber Shop</p></div></div>}>
      <div><p className="text-sm font-medium text-emerald-400">{mode === 'login' ? 'Bem-vindo de volta' : 'Comece agora'}</p><h2 className="mt-3 text-3xl font-bold tracking-tight text-white sm:text-4xl">{mode === 'login' ? 'Entre na sua conta' : 'Crie sua conta'}</h2><p className="mt-3 text-sm leading-6 text-slate-400 sm:text-base">Comece a cuidar melhor do seu negocio hoje.</p><form onSubmit={submit} className="mt-8 space-y-4"><>{mode === 'signup' && <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium text-slate-200">Nome do responsavel<input required value={responsibleName} onChange={(event) => setResponsibleName(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-800 bg-slate-900 px-4 py-3 text-base text-white outline-none ring-emerald-400 transition placeholder:text-slate-600 focus:ring-2" placeholder="Seu nome completo" /></label><label className="block text-sm font-medium text-slate-200">Nome da barbearia<input required value={barbershopName} onChange={(event) => setBarbershopName(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-800 bg-slate-900 px-4 py-3 text-base text-white outline-none ring-emerald-400 transition placeholder:text-slate-600 focus:ring-2" placeholder="Ex.: Barbearia Jacobina" /></label><label className="block text-sm font-medium text-slate-200 sm:col-span-2">WhatsApp<input required type="tel" value={whatsapp} onChange={(event) => setWhatsapp(maskWhatsApp(event.target.value))} className="mt-2 w-full rounded-xl border border-slate-800 bg-slate-900 px-4 py-3 text-base text-white outline-none ring-emerald-400 transition placeholder:text-slate-600 focus:ring-2" placeholder="(00) 00000-0000" /></label></div>}</><label className="block text-sm font-medium text-slate-200">E-mail<input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-800 bg-slate-900 px-4 py-3.5 text-base text-white outline-none ring-emerald-400 transition placeholder:text-slate-600 focus:ring-2" placeholder="voce@barbearia.com" /></label><PasswordField label="Senha" value={password} onChange={setPassword} placeholder="Sua senha" />{mode === 'login' && <button type="button" onClick={() => window.location.assign('/esqueci-senha')} className="text-xs font-semibold text-emerald-400 transition hover:text-emerald-300">Esqueci minha senha</button>}{message && <p className="rounded-lg border border-amber-400/20 bg-amber-400/10 px-3 py-2 text-sm text-amber-200">{message}</p>}<button disabled={loading} type="submit" className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-400 py-3.5 text-base font-semibold text-slate-950 transition hover:bg-emerald-500 disabled:opacity-60">{loading ? 'Aguarde...' : mode === 'login' ? 'Entrar' : 'Criar conta'}<ArrowRight size={17} /></button></form><div className="my-5 flex items-center gap-3 text-xs text-slate-600"><span className="h-px flex-1 bg-slate-800" />ou<span className="h-px flex-1 bg-slate-800" /></div><button type="button" onClick={() => void continueWithGoogle()} disabled={loading} className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-700 bg-slate-900 py-3.5 text-sm font-semibold text-white transition hover:border-emerald-500/50 hover:bg-slate-800 disabled:opacity-60"><GoogleIcon />Continuar com o Google</button><button type="button" onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setMessage('') }} className="mt-7 flex w-full items-center justify-center gap-1 text-sm text-slate-400">{mode === 'login' ? 'Ainda nao tem uma conta?' : 'Ja tem uma conta?'}<span className="font-semibold text-emerald-400">{mode === 'login' ? 'Cadastre-se' : 'Entrar'}</span></button><div className="mt-8 flex items-center justify-center gap-2 text-xs text-slate-500"><Check size={14} className="text-emerald-400" /> Seus dados ficam protegidos pelo Supabase</div></div>
    </AuthLayout>
    {recoveryOpen && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm"><section role="dialog" aria-modal="true" className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl"><div className="flex items-start justify-between"><div><div className="mb-3 inline-flex rounded-xl bg-emerald-500/10 p-2 text-emerald-400"><KeyRound className="h-5 w-5" /></div><h3 className="text-xl font-bold text-white">Recuperar acesso</h3><p className="mt-1 text-sm text-slate-400">Enviamos um codigo de recuperacao para seu e-mail. Digite o codigo recebido e defina sua nova senha.</p></div><button type="button" onClick={() => setRecoveryOpen(false)} aria-label="Fechar recuperacao" className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 hover:text-white"><X className="h-5 w-5" /></button></div>{recoveryStep === 'email' ? <form onSubmit={sendRecoveryEmail} className="mt-6 space-y-4"><label className="block text-sm text-slate-300">E-mail<input required type="email" value={recoveryEmail} onChange={(event) => setRecoveryEmail(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white outline-none focus:border-emerald-500" placeholder="voce@barbearia.com" /></label><button disabled={recoveryLoading} className="w-full rounded-xl bg-emerald-400 py-3 font-bold text-slate-950 hover:bg-emerald-300 disabled:opacity-60">{recoveryLoading ? 'Enviando...' : 'Enviar link seguro'}</button></form> : <form onSubmit={updateRecoveredPassword} className="mt-6 space-y-4"><label className="block text-sm text-slate-300">Codigo recebido (opcional ao abrir pelo link)<input value={recoveryToken} onChange={(event) => setRecoveryToken(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white outline-none focus:border-emerald-500" placeholder="Cole o codigo recebido" /></label><PasswordField label="Nova senha" value={recoveryPassword} onChange={setRecoveryPassword} placeholder="Minimo de 6 caracteres" showStrength /><button disabled={recoveryLoading} className="w-full rounded-xl bg-emerald-400 py-3 font-bold text-slate-950 hover:bg-emerald-300 disabled:opacity-60">{recoveryLoading ? 'Atualizando...' : 'Atualizar senha'}</button></form>}{recoveryMessage && <p className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-3 text-sm text-emerald-200">{recoveryMessage}</p>}</section></div>}
  </>
}
