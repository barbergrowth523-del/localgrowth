...
const COMPATIBILITY_KEY = 'cookies_consent'

type Consent = 'accepted' | 'declined'

function readConsent(): Consent | null {
  const value = window.localStorage.getItem(CONSENT_KEY) ?? window.localStorage.getItem(COMPATIBILITY_KEY)
...
export function CookieBanner() {
  ...
  function saveConsent(value: Consent) {
    ...
    window.localStorage.setItem(COMPATIBILITY_KEY, value)
    setConsent(value)
    window.dispatchEvent(new Event('prontusfy-cookie-consent'))
  }

  // A saved choice closes the banner immediately. Preferences remain available in browser storage.
