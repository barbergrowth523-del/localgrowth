...
type Availability = { capacity: number; booked: Record<string, number> }
...
export default function PublicBookingForm({ barbearia }: { barbearia: string }) {
  ...
  const [availability, setAvailability] = useState<Availability>({ capacity: 1, booked: {} })
  ...
  useEffect(() => {
    ...
    fetch('/api/agendar?barbearia=' + encodeURIComponent(barbearia) + '&date=' + selectedDate)
      ...
      .then((data: Availability) => setAvailability({ capacity: Math.max(1, data.capacity ?? 1), booked: data.booked ?? {} }))
      .catch(() => setAvailability({ capacity: 1, booked: {} }))
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch('/api/agendar', {
      ...
      body: JSON.stringify({ barbearia, nome, telefone, servicoId, servico: services.find((service) => service.id === servicoId)?.nome, date: selectedDate, time: selectedTime }),
    })
