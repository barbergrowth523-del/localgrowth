...
type DayOption = { key: string; label: string; dateLabel: string; date: Date }
const weekLabels = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sab']
const fallbackServices: Service[] = [{ id: 'fallback-corte', nome: 'Corte', preco: 60, duracao_minutos: 30 }, { id: 'fallback-barba', nome: 'Barba', preco: 40, duracao_minutos: 30 }]
...
function dateLabel(date: Date) { return String(date.getDate()).padStart(2, '0') + '/' + String(date.getMonth() + 1).padStart(2, '0') }
...
export default function PublicBookingForm({ barbearia }: { barbearia: string }) {
  ...
  const days = useMemo<DayOption[]>(() => Array.from({ length: 7 }, (_, index) => { const date = new Date(); date.setHours(12, 0, 0, 0); date.setDate(date.getDate() + index); return { key: dateKey(date), label: index === 0 ? 'Hoje' : index === 1 ? 'Amanha' : weekLabels[date.getDay()], dateLabel: dateLabel(date), date } }), [])

  useEffect(() => {
    setLoadingServices(true)
    fetch('/api/servicos?barbearia=' + encodeURIComponent(barbearia)).then((response) => response.ok ? response.json() : { services: [] }).then((data: { services?: Service[] }) => { const loaded = data.services ?? []; const available = loaded.length ? loaded : fallbackServices; setServices(available); if (available.length) setServicoId(available[0].id) }).catch(() => setServices([])).finally(() => setLoadingServices(false))
  }, [barbearia])
  ...
  return <main className="min-h-screen bg-slate-950 px-5 py-8 text-white sm:py-12"><section className="mx-auto w-full max-w-xl rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-2xl sm:p-8"><div className="mb-8 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-500/10 text-emerald-400"><CalendarCheck className="h-6 w-6" /></div><h1 className="mt-4 text-2xl font-bold">Agende na {nomeBarbearia}</h1><p className="mt-2 text-sm text-slate-400">Escolha o melhor horario para o seu proximo corte.</p></div><div className="mb-8 grid grid-cols-4 gap-2 text-center text-[10px] font-semibold uppercase tracking-wide text-slate-500 sm:grid-cols-4"><span className="text-emerald-400">1. Identificacao</span><span>2. Servico</span><span>3. Dia e horario</span><span>4. Confirmacao</span></div><form onSubmit={submit} className="space-y-7"><section><div className="mb-3 flex items-center gap-2"><UserRound className="h-4 w-4 text-emerald-400" /><h2 className="text-sm font-semibold text-white">1. Identificacao</h2></div><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm text-slate-300">Nome completo<input required value={nome} onChange={(event) => setNome(event.target.value)} placeholder="Ex: Joao Silva" className="mt-2 w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3.5 text-white placeholder:text-slate-600 outline-none focus:border-emerald-500" /></label><label className="block text-sm text-slate-300">WhatsApp / Telefone<input required minLength={8} value={telefone} onChange={(event) => setTelefone(event.target.value)} placeholder="(00) 00000-0000" className="mt-2 w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3.5 text-white placeholder:text-slate-600 outline-none focus:border-emerald-500" /></label></div></section><section><h2 className="mb-3 text-sm font-semibold text-white">2. Escolha o servico</h2>{loadingServices ? <p className="rounded-xl border border-slate-800 bg-slate-950 p-4 text-sm text-slate-500">Carregando servicos...</p> : services.length ? <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{services.map((service) => <button type="button" key={service.id} onClick={() => setServicoId(service.id)} className={'rounded-2xl border p-4 text-left transition ' + (servicoId === service.id ? 'border-emerald-500 bg-emerald-500/10' : 'border-slate-800 bg-slate-950 hover:border-slate-700')}><p className="font-semibold text-white">{service.nome}</p><p className="mt-2 text-sm text-emerald-300">R$ {Number(service.preco).toFixed(2).replace('.', ',')}</p><p className="mt-1 text-xs text-slate-500">{service.duracao_minutos} minutos</p></button>)}</div> : <p className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-300">Nenhum servico cadastrado pela barbearia.</p>}</section><section><h2 className="mb-3 text-sm font-semibold text-white">3. Escolha o dia e horario</h2><div className="flex gap-2 overflow-x-auto pb-2">{days.map((day) => <button type="button" key={day.key} onClick={() => { setSelectedDate(day.key); setSelectedTime('') }} className={'min-w-[76px] rounded-xl border px-3 py-3 text-center transition ' + (selectedDate === day.key ? 'border-emerald-500 bg-emerald-500/10 text-emerald-300' : 'border-slate-800 bg-slate-950 text-slate-400 hover:border-slate-700')}><span className="block text-xs font-bold">{day.label}</span><span className="mt-1 block text-xs">{day.dateLabel}</span></button>)}</div><div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-4">{timeOptions.map((time) => <button type="button" key={time} onClick={() => setSelectedTime(time)} className={'rounded-lg border px-2 py-3 text-sm font-semibold transition ' + (selectedTime === time ? 'border-emerald-500 bg-emerald-500 text-slate-950' : 'border-slate-800 bg-slate-950 text-slate-300 hover:border-emerald-500/60')}>{time}</button>)}</div></section>{message && <p className="rounded-xl border border-rose-500/20 bg-rose-500/10 px-4 py-3 text-sm text-rose-300">{message}</p>}<button disabled={loading || loadingServices || !services.length} type="submit" className="w-full rounded-xl bg-emerald-500 px-5 py-4 text-base font-bold text-slate-950 transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-60">{loading ? 'Salvando...' : 'Confirmar agendamento'}</button></form></section></main>
}
