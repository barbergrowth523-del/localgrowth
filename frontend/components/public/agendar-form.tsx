...
export default function PublicBookingForm({ barbearia }: { barbearia: string }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/servicos?barbearia=' + encodeURIComponent(barbearia)).then((response) => response.ok ? response.json() : { services: [] }).then((data: { services?: Service[] }) => { const loaded = data.services ?? []; setServices(loaded); if (loaded.length) setServicoId(loaded[0].id) }).catch(() => setServices([])).finally(() => setLoadingServices(false))
  }, [barbearia])
